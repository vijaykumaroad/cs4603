...
DATABRICKS_EMAIL = os.environ.get("DATABRICKS_EMAIL")
...
def setup_mlflow(experiment_name: str):
    ...
    tracking_uri = (os.environ.get("MLFLOW_TRACKING_URI") or "http://127.0.0.1:5000").strip()
    # tracking_uri = os.environ.get("MLFLOW_TRACKING_URI")
    mlflow.set_tracking_uri(tracking_uri)

    if "databricks" in tracking_uri.lower():
        if not DATABRICKS_EMAIL:
            raise ValueError("Missing DATABRICKS_EMAIL. Check your .env file")
